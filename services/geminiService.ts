...
export const generateSticker = async (prompt: string, style: StickerStyle, referenceImage?: string): Promise<string> => {
  if (!API_KEY) {
  ...
  // Construct prompt based on whether an image is provided
  let instruction = `Create a single, isolated die-cut sticker design of ${prompt}.`;
  
  if (referenceImage) {
    instruction = `Transform this reference image into a die-cut sticker design.`;
    if (prompt && prompt.trim()) {
      instruction += ` Modifications: ${prompt}.`;
    }
  }
